...
def download_song(youtube_link, song_name, album_name):
	try:
		...
		song_name = re.sub(r'[^\x00-\x7F(){}[\]]+', '', remove_invalid_chars(song_name))
		album_name = re.sub(r'[^\x00-\x7F(){}[\]]+', '', remove_invalid_chars(album_name))

		# Normalize the file path to use forward slashes and remove "./"
		file_path = os.path.join("downloaded-songs", f"{song_name} - {album_name}.mp4").replace("\\", "/")

		if os.path.exists(file_path):
			print(f"Skipping download for {song_name}. File already exists.")
		else:
			try:
				audio_file = audio_stream.download(output_path="downloaded-songs", filename=f"{song_name} - {album_name}.mp4")
...
jump_to_download = input("Jump to downloading songs using YouTube links from songs.json? (Y/N): ").strip().lower()

if jump_to_download == 'y':
